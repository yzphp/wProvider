...
class Service
{
     ...
     */
    public function instance($name, $authorizerAppid, $type = 'WeMini')
    {
        $className = "WeOpen\\{$type}\\" . ucfirst(strtolower($name));
        Tools::writeLog(['instance'=>func_get_args()]);
    ...
    {
        $result = json_decode(Tools::post($url, $buildToJson ? Tools::arr2json($data) : $data), true);
        if(isset($result['errcode'])){
            $result['errmsg'] = isset(DataError::$message[$result['errcode']]) ? DataError::$message[$result['errcode']].'('.@$result['errmsg'].')' : @$result['errmsg'];
        }
        return $result;
    }
    ...
    {
        $result = json_decode(Tools::get($url), true);
        if(isset($result['errcode'])){
            $result['errmsg'] = isset(DataError::$message[$result['errcode']]) ? DataError::$message[$result['errcode']].'('.@$result['errmsg'].')' : @$result['errmsg'];
        }
        return $result;
